...
generated_at_local: {rec.yaml_quote(datetime.now().isoformat(timespec="seconds"))}
run_name: {rec.yaml_quote(run_name)}
output_dir: {rec.yaml_quote(out_dir.replace(os.sep, "/"))}
command: {rec.yaml_quote(cmd)}
...
coordinate_convention:
  output_frame: {rec.yaml_quote("ROS body frame: x forward, y left, z up")}
  carla_to_output_position_vector: {rec.yaml_quote("[x, -y, z]")}
  carla_to_output_imu_angular_velocity: {rec.yaml_quote("[-x, y, -z]")}
  note: {rec.yaml_quote("LiDAR packets are merged without recorder-side deskew; each point keeps its partial packet timestamp.")}

camera:
  enabled: false

lidar:
  name: {rec.yaml_quote("lidar")}
  blueprint: {rec.yaml_quote("sensor.lidar.ray_cast")}
  directory: {rec.yaml_quote("lidar")}
  data_directory: {rec.yaml_quote("lidar/data")}
  timestamps_file: {rec.yaml_quote("lidar/timestamps.csv")}
...
imu:
  name: {rec.yaml_quote("imu")}
  blueprint: {rec.yaml_quote("sensor.other.imu")}
  directory: {rec.yaml_quote("imu")}
  data_directory: {rec.yaml_quote("imu/data")}
  timestamps_file: {rec.yaml_quote("imu/timestamps.csv")}
  data_format: {rec.yaml_quote("raw float64 row-major 1x8")}
  sample_stride_bytes: 64
  fields: ["accel_x", "accel_y", "accel_z", "gyro_x", "gyro_y", "gyro_z", "compass", "timestamp"]
  accel_units: {rec.yaml_quote("m/s^2")}
  gyro_units: {rec.yaml_quote("rad/s")}
  compass_units: {rec.yaml_quote("rad, CARLA-provided compass")}
  timestamp_field: {rec.yaml_quote("absolute CARLA elapsed_seconds")}
  data_source: {rec.yaml_quote("synthetic_kinematic")}
  noise_model: {rec.yaml_quote("ideal_zero_noise_bias")}
  noise_spec:
...
def append_scenario_summary(path: str, summary: Dict[str, Any]) -> None:
    ...
    text = "\nscenario:\n"
    text += f"  mode: {rec.yaml_quote('rotation-around-object')}\n"

...
camera:
  name: "camera"
  directory: "camera"
  data_directory: "camera/data"
  timestamps_file: "camera/timestamps.txt"
  data_format: "png"
  color_mode: "{args.camera_color}"
...
lidar:
  ...
  timestamps_file: "lidar/timestamps.txt"
  ...
  point_count_note: "CARLA ray_cast returns hit points only; requested rays/sweep is tuned to produce about 50k returned points in typical maps."
  ...
  ring_calculation: "round((atan2(z, hypot(x, y))_deg - lower_fov) / (upper_fov - lower_fov) * (channels - 1))"
  base_link_to_sensor_ros:
    x: {lidar_tf_ros["x"]:.9f}
    y: {lidar_tf_ros["y"]:.9f}
    z: {lidar_tf_ros["z"]:.9f}
...
imu:
  ...
  timestamps_file: "imu/timestamps.txt"
...
def append_route_summary(path: str, summary: Dict[str, Any]) -> None:
    ...
    text += f"  route_imu_mode: \"{summary['route_imu_mode']}\"\n"
...
def yaml_scalar(value: Any) -> str:
    ...
    if isinstance(value, int):
        return str(value)
    return f"\"{str(value).replace(chr(34), chr(39))}\""
...
def main() -> None:
    ...
    try:
        ...
        f_cam_ts = open(os.path.join(camera_dir, "timestamps.txt"), "w", encoding="ascii", buffering=1)
        ...
        while target_ticks is None or tick_idx < target_ticks:
            ...
            if imu_meas is None:
                ...
                f_imu_ts.write(f"{seq_imu:06d} {format_float(float(imu_meas.timestamp))} {int(imu_meas.frame)}\n")
            ...
            if lidar_meas is None:
                ...
                if len(lidar_packets) == lidar_sweep_ticks:
                    ...
                    f_lidar_ts.write(
                        f"{seq_lidar:06d} {format_float(end_ts)} {end_frame} "
                        f"{format_float(start_ts)} {start_frame}\n"
                    )
            ...
            for cam_meas in camera_buf.pop_all_ready(world_frame, warmup_frame):
                ...
                f_cam_ts.write(f"{seq_camera:06d} {format_float(float(cam_meas.timestamp))} {int(cam_meas.frame)}\n")
